...
export class MailsService {
  ...
  /**
   * `autologinTokensByAddresseeId` is generated by the caller (`AuthService`
   * being a dependency of `MailsModule` would introduce a module cycle) and
   * embedded in each addressee's `conversationUrl`, letting them access the
   * conversation directly from the email without going through `/login`.
   */
  async sendNewMessageNotifMail(
    message: Message,
    addressees: User[],
    autologinTokensByAddresseeId: Record<string, string>
  ) {
    this.logger.log(
  ...
  /**
   * `autologinToken` is generated by the caller for the same module-cycle reason as
   * `sendNewMessageNotifMail` above.
   */
  async sendCheckinInvitationMail(
    addressee: User,
    ...
    this.logger.log(
      `Sending checkin invitation mail to addressee with email ${addressee.email}`
    );
    const checkinUrl = `${process.env.FRONT_URL}/backoffice/checkin/${conversationId}?autologinToken=${encodeURIComponent(autologinToken)}`;
  ...
  async sendCheckinRelanceMail(
    ...
    this.logger.log(
      `Sending checkin relance mail to addressee with email ${addressee.email}`
    );
    const checkinUrl = `${process.env.FRONT_URL}/backoffice/checkin/${conversationId}?autologinToken=${encodeURIComponent(autologinToken)}`;
  ...
  /**
   * `autologinToken` is generated by the caller for the same module-cycle reason as
   * `sendNewMessageNotifMail` above. Unlike `sendCheckinInvitationMail`/
   * `sendCheckinRelanceMail`, this is triggered synchronously from the note-sending
   * flow rather than a daily cron, so `conversationUrl` points at the conversation
   * itself (like `sendNewMessageNotifMail`), not the checkin screen.
   */
  async sendCheckinNoteMail(
    ...
    this.logger.log(
      `Sending checkin note mail to addressee with email ${addressee.email}`
    );
    const conversationUrl = `${process.env.FRONT_URL}/backoffice/messaging?userId=${otherParticipant.id}&autologinToken=${encodeURIComponent(autologinToken)}`;
